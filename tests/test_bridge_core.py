...
class DummyMax:
    ...
    async def set_reaction(self, chat_id: str, msg_id: str, emoji):
        self.reaction_calls.append((chat_id, msg_id, emoji))
        return True
...
class DummyTelegram:
    ...
    async def set_message_reaction(self, tg_msg_id: int, reaction: str, *, flow_id=None) -> bool:
        # Отдельный список: статусные реакции (👀/👍/👎) на исходящих не должны
        # смешиваться с ожиданиями calls в тестах доставки.
        self.reactions.append((tg_msg_id, reaction))
        return True
...
@pytest.mark.asyncio
async def test_on_max_reaction_update_sets_same_reaction_in_telegram():
    repo = DummyRepo()
    ...
    # Текст сообщения править нельзя: plaintext не хранится, правка стёрла бы
    # само сообщение. Реакция из MAX отражается реакцией на копии в Telegram.
    assert tg.reactions == [(321, "👍")]
    assert tg.calls == []


@pytest.mark.asyncio
async def test_on_tg_reaction_action_mirrors_reaction_into_max():
    repo = DummyRepo()
    repo.reply_mappings[321] = SimpleNamespace(
        tg_msg_id=321,
        max_chat_id="-70000000000003",
        max_msg_id="mx-77",
        tg_topic_id=99,
    ...
    await bridge._on_tg_reaction_action(TelegramReactionAction(tg_msg_id=321, emoji="👍"))

    assert max_adapter.reaction_calls == [("-70000000000003", "mx-77", "👍")]


@pytest.mark.asyncio
async def test_on_tg_reaction_action_clears_reaction_in_max():
    repo = DummyRepo()
    repo.reply_mappings[321] = SimpleNamespace(
        tg_msg_id=321,
        max_chat_id="-70000000000003",
    ...
    await bridge._on_tg_reaction_action(TelegramReactionAction(tg_msg_id=321, emoji=None))

    assert max_adapter.reaction_calls == [("-70000000000003", "mx-77", None)]


@pytest.mark.asyncio
async def test_on_tg_reaction_action_ignores_unknown_message():
    repo = DummyRepo()
    repo.reply_mappings[321] = None
    max_adapter = DummyMax()
    bridge = make_bridge(repo=repo, max_adapter=max_adapter)

    await bridge._on_tg_reaction_action(TelegramReactionAction(tg_msg_id=321, emoji="👍"))

    assert max_adapter.reaction_calls == []
...
@pytest.mark.asyncio
async def test_on_tg_reply_does_not_send_sender_name_to_max():
    repo = DummyRepo()
    max_adapter = DummyMax()
    tg_adapter = DummyTelegram()
    bridge = BridgeCore(
...
@pytest.mark.asyncio
async def test_on_tg_reply_rejects_binding_removed_from_max_whitelist():
    repo = DummyRepo()
    max_adapter = DummyMax()
    tg_adapter = DummyTelegram()
    config = SimpleNamespace(
        ...
        is_allowed_max_chat_id=lambda max_chat_id: False,
    )
    bridge = BridgeCore(
        config=config,
        repo=repo,
    ...
    await bridge._on_tg_reply(
        topic_id=99,
        tg_msg_id=556,
        text="Не должно уйти в удалённый whitelist чат",
...
async def test_max_watchdog_reports_egress_down_without_restart(tmp_path):
    ...
    task = asyncio.create_task(
        bridge_background.run_max_watchdog(
            ...
            egress_startup_grace_seconds=0,
...
@pytest.mark.asyncio
async def test_max_watchdog_suppresses_home_proxy_down_during_startup_grace(tmp_path):
    class OfflineHomeProxyMax(DummyMax):
        def __init__(self):
            super().__init__()
            self.egress_status = {"max_egress_active": "home_ru_proxy"}
    ...
    task = asyncio.create_task(
        bridge_background.run_max_watchdog(
            ...
            check_interval=0,
            egress_probe_interval=0,
            egress_startup_grace_seconds=60,
            self_heal_grace_seconds=0,
            self_heal_state_path=tmp_path / "self-heal.json",
    ...
    issue = snapshot.subsystems["max_link"].issue
    assert issue is not None
    assert issue.code == "max_egress_startup_wait"
    assert alerts == []
...
async def test_on_tg_reply_queues_definite_unsent_text_after_max_error():
    ...
    assert queued.max_chat_id == "-70000000000003"
    assert queued.text == "Проверка ошибки"
