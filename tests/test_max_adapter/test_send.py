...
@pytest.mark.asyncio
async def test_start_pauses_reconnect_after_auth_rate_limit(tmp_path, monkeypatch, caplog):
    class RateLimitedStartClient(StartClient):
        async def start(self):
            await self.start_handler()
            raise RuntimeError(
                "ApiError opcode=17 error=limit.violate "
                "message=Попробуйте позже Слишком много попыток"
            )
    ...
    adapter = AdapterHarness(
        ...
        session_name="session",
        tmp_dir=str(tmp_path / "tmp"),
        backend=LifecycleBackend(RateLimitedStartClient()),
    )

    async def fail_if_sleep_called(_delay):
        raise AssertionError("rate-limited auth must not enter reconnect sleep")
    ...
    assert adapter.get_last_issue().kind == "max_auth_rate_limited"
    assert "max.adapter.reconnect_paused" in caplog.text
