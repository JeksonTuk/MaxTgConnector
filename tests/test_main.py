...
from src.startup.composition import _build_max_health_payload
...
def test_rate_limited_max_health_payload_does_not_promise_reconnect():
    payload = _build_max_health_payload(
        MaxIssue(
            kind=MAX_AUTH_RATE_LIMITED_KIND,
            summary="MAX временно ограничил запросы кода авторизации",
            raw_error="safe error",
            requires_reauth=False,
        )
    )

    assert "остановлены" in payload["auto_recovery"]
    assert "продолжит reconnect" not in payload["auto_recovery"]
