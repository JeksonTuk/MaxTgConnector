...
class MaxAdapter:
    ...
    async def set_reaction(
        ...
        """Поставить или снять реакцию на сообщение в MAX.

        emoji=None снимает реакцию. MAX принимает числовые идентификаторы,
        поэтому нечисловые chat_id/msg_id отбрасываются без запроса.
        """
        # Живой клиент PyMax лежит в connection-state; backend умеет только
        # создавать новые клиенты.
        adapter = getattr(self._state.connection, "client", None)
        client = getattr(adapter, "raw_client", None)
        if client is None:
            log_event(
                logger,
                logging.WARNING,
                "max.reaction.skipped",
                stage="reaction",
                outcome="skipped",
        ...
        try:
            ...
            log_event(
                ...
                stage="reaction",
                outcome="skipped",
                reason="non_numeric_id",
            )
            return False

        try:
            if emoji:
                result = await client.add_reaction(numeric_chat_id, numeric_msg_id, emoji)
            else:
                # На снятие MAX может ответить пустым телом: отсутствие
                # исключения и есть успех.
                await client.remove_reaction(numeric_chat_id, numeric_msg_id)
                return True
