...
#: Типы link, которые действительно означают пересылку чужого сообщения.
#: Всё остальное (REPLY и незнакомые типы) — связь внутри чата: текст берём
#: у самого сообщения, иначе в Telegram уезжает цитата вместо ответа.
FORWARD_LINK_TYPES = {"CHANNEL", "FORWARD", "FORWARDED"}
...
class RawPayloadParser:
    ...
    def _extract_forwarded_payload(self, message) -> Optional[ForwardedPayload]:
        ...
        if link:
            ...
            if linked_message is not None and link_type in FORWARD_LINK_TYPES:
