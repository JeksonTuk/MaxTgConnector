...
class Repository:
    ...
    async def create_or_claim_pending_outbound(
        self, job: PendingOutboundMessage, *, lease_until: int, now: Optional[int] = None
    ) -> tuple[Optional[PendingOutboundMessage], bool]:
        return await self._pending_outbound.create_or_claim_pending_outbound(
            job, lease_until=lease_until, now=now
        )

    async def recover_inflight_outbound_as_unknown(self, *, now: Optional[int] = None) -> int:
        return await self._pending_outbound.recover_inflight_outbound_as_unknown(now=now)
    ...
    async def mark_pending_outbound_unknown(
        self, job_id: int, *, error: str, now: Optional[int] = None
    ):
        await self._pending_outbound.mark_pending_outbound_unknown(job_id, error=error, now=now)
    ...
    async def queue_outbound_reaction(self, **kwargs) -> int:
        return await self._pending_outbound.queue_outbound_reaction(**kwargs)

    async def get_due_pending_reactions(self, *, now: Optional[int] = None, limit: int = 20):
        return await self._pending_outbound.get_due_pending_reactions(now=now, limit=limit)

    async def mark_pending_reaction_retry(self, reaction_id: int, *, error: str,
                                          next_attempt_at: int, now: Optional[int] = None):
        await self._pending_outbound.mark_pending_reaction_retry(
            reaction_id, error=error, next_attempt_at=next_attempt_at, now=now
        )
    ...
    async def mark_pending_reaction_delivered(
        ...
        *,
        ...
        return await self._pending_outbound.mark_pending_reaction_delivered(
            reaction_id,
    ...
    async def mark_pending_reaction_current_delivered(
        ...
        *,
        ...
        await self._pending_outbound.mark_pending_reaction_current_delivered(
            topic_id=topic_id, tg_msg_id=tg_msg_id, reaction=reaction, now=now
        )
