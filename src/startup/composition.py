...
from src.adapters.max.errors import MAX_AUTH_RATE_LIMITED_KIND
...
def _build_max_health_payload(issue: MaxIssue) -> dict:
    if issue.kind == MAX_AUTH_RATE_LIMITED_KIND:
        operator_hint = "Дождись снятия временного лимита MAX, затем запусти bridge один раз вручную."
        auto_recovery = "Автоматические попытки авторизации остановлены, чтобы не продлевать лимит MAX."
    else:
        operator_hint = (
    ...
    *,
    ...
    try:
        ...
        tg_adapter = TelegramAdapter(
            ...
            outbound_enabled=cfg.bridge.outbound_enabled,
            max_file_size_mb=cfg.bridge.max_file_size_mb,
        )
        ...
        async def on_max_ready():
            ...
            if started_once:
                return
            started_once = True
            # Уведомление «Maxgram запущен» отключено по решению владельца
            # (14.09): при каждом обновлении оно приходило в личку. Сообщения
            # о сбоях и восстановлении (_emit_health_change выше) остаются.
            await startup_tests_runner(logger)
        ...
        async with asyncio.TaskGroup() as tg:
            ...
            tg.create_task(
                ...
                # message_reaction Telegram шлёт только по явному запросу и
                # запоминает набор на своей стороне до следующего getUpdates.
                dp.start_polling(
                    bot,
            ...
            tg.create_task(bridge.run_pending_reactions(), name="pending_reactions")
            tg.create_task(
