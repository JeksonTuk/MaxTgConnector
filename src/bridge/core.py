...
class BridgeCore:
    def __init__(
        ...
        register_reaction = getattr(self._tg, "on_reaction_action", None)
        if callable(register_reaction):
            register_reaction(self._on_tg_reaction_action)
    ...
    async def _on_max_reaction_update(self, event: MaxReactionUpdate) -> None:
        """Реакция в MAX → такая же реакция на копии сообщения в Telegram.

        Текст сообщения править нельзя: Telegram требует прислать его
        целиком, а plaintext намеренно не хранится (privacy first). Прежняя
        реализация заменяла текст строкой со счётчиками и тем самым стирала
        само сообщение. Telegram допускает одну реакцию бота на сообщение,
        поэтому отражается последняя реакция из MAX.
        """
        tg_msg_id = await self._repo.get_tg_msg_by_max(event.chat_id, event.message_id)
        if tg_msg_id is None:
            return

        reaction = (event.reaction or "").strip()
        if not reaction:
            for counter in event.counters or []:
                emoji = str(counter.get("emoji") or "").strip()
                if emoji:
                    reaction = emoji
                    break
        if not reaction:
        ...
        setter = getattr(self._tg, "set_message_reaction", None)
        if not callable(setter):
            return
        await setter(tg_msg_id, reaction)

    async def _on_tg_reaction_action(self, action: TelegramReactionAction) -> None:
        """Реакция в Telegram → та же реакция на исходном сообщении в MAX.

        Отражаются только сообщения, которые мост сам доставил: связка
        tg_msg_id → (max_chat_id, max_msg_id) живёт в tg_reply_map.
        """
        mapping = await self._repo.get_tg_reply_mapping(action.tg_msg_id)
        if mapping is None:
            return

        setter = getattr(self._max, "set_reaction", None)
        if not callable(setter):
            return

        ok = await setter(mapping.max_chat_id, mapping.max_msg_id, action.emoji)
        log_event(
            logger,
            logging.INFO,
            "bridge.outbound.reaction_mirrored",
            direction="outbound",
            stage="reaction",
            outcome="delivered" if ok else "failed",
            tg_msg_id=action.tg_msg_id,
            max_chat_id=mapping.max_chat_id,
            max_msg_id=mapping.max_msg_id,
            cleared=action.emoji is None,
        )
    ...
    async def run_pending_outbound_messages(
        ...
        await bridge_outbound_retry.run_pending_outbound_messages(
            ...
            lease_seconds=lease_seconds,
            ttl_seconds=ttl_seconds,
            outbound_enabled=getattr(self._cfg.bridge, "outbound_enabled", True),
            is_max_chat_allowed=getattr(self._cfg, "is_allowed_max_chat_id", None),
        )

    async def run_pending_reactions(self, poll_interval: int = 30):
        await bridge_outbound_retry.run_pending_reactions(
            repo=self._repo,
            tg=self._tg,
            poll_interval=poll_interval,
        )
