...
from .message_context import forwarded_context_marker, sender_prefix as compose_sender_prefix
...
def _max_chat_allowed(cfg: AppConfig, chat_id: str) -> bool:
    checker = getattr(cfg, "is_allowed_max_chat_id", None)
    return bool(checker(chat_id)) if callable(checker) else True
...
async def mark_removed_message_in_topic(
    *,
    tg: TelegramBridgePort,
    repo: Repository,
    msg: MaxMessage,
    ...
    """Пометить удалённое в MAX сообщение ответом на его копию в Telegram.

    MAX присылает удаление как то же сообщение со статусом REMOVED. Без этой
    ветки мост отправлял в топик отдельное «[Сообщение удалено]», не
    связанное с оригиналом. Текст исходного сообщения нигде не хранится,
    поэтому редактирование копии не подходит — оно стёрло бы сам текст.

    Возвращает True, если событие обработано и обычная доставка не нужна.
    """
    base_msg_id = media_retry.removed_base_max_msg_id(msg.msg_id, msg.status)
    if not base_msg_id:
        return False

    original_tg_msg_id = await repo.get_tg_msg_by_max(msg.chat_id, base_msg_id)
    if original_tg_msg_id is None:
        # Оригинал не доставлялся (например, удалён до запуска моста):
        # отдельное сообщение об удалении смысла не имеет.
        log_event(
            logger,
            logging.INFO,
            "bridge.inbound.removed_marked",
            flow_id=flow_id,
            direction="inbound",
            stage="routing",
            outcome="skipped",
    ...
    sent_id = await tg.send_text(
        binding.tg_topic_id,
        "❌ Это сообщение удалено в MAX",
        reply_to_msg_id=original_tg_msg_id,
        flow_id=flow_id,
    )
    log_event(
        logger,
        ...
        direction="inbound",
        stage="routing",
        outcome="delivered" if sent_id else "failed",
        max_chat_id=msg.chat_id,
        max_msg_id=msg.msg_id,
        tg_topic_id=binding.tg_topic_id,
        tg_msg_id=original_tg_msg_id,
    )
    return True
...
async def forward_to_telegram(
    *,
    ...
    sender_prefix = compose_sender_prefix(
        is_own=msg.is_own,
        is_dm=msg.is_dm,
    ...
    # У медиа без текста подпись стоит одна: висящее двоеточие тут лишнее.
    media_caption = body_text or (
        sender_prefix.strip().rstrip(":") if msg.attachments else ""
    )
...
async def handle_max_message(
    *,
    ...
    # Explicit MAX whitelist is checked before dedup, user cache, title resolve,
    # media recovery and every Telegram side effect.
    if not _max_chat_allowed(cfg, msg.chat_id):
        log_event(
            logger,
            logging.INFO,
            "bridge.inbound.forward_finished",
    ...
    if await mark_removed_message_in_topic(
        tg=tg,
        repo=repo,
        msg=msg,
        flow_id=flow_id,
