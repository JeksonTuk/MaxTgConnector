...
_REMOVED_STATUS_SUFFIXES = (":REMOVED", ":MESSAGESTATUS.REMOVED")
...
def removed_base_max_msg_id(max_msg_id: object, status: object | None = None) -> str | None:
    """Исходный max_msg_id для события удаления.

    MAX присылает удаление как то же сообщение с суффиксом статуса
    (``<id>:REMOVED``). Базовый id нужен, чтобы пометить уже доставленную
    копию в Telegram, а не отправлять отдельное сообщение об удалении.
    """
    text = str(max_msg_id or "").strip()
    if not text:
        return None
    ...
    if "." in status_text:
        status_text = status_text.rsplit(".", 1)[-1]
    upper_text = text.upper()
    for suffix in _REMOVED_STATUS_SUFFIXES:
        if upper_text.endswith(suffix):
            return text[: -len(suffix)] or None
    return text if status_text == "REMOVED" else None
