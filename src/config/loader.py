...
class AppConfig:
    ...
    def is_allowed_max_chat_id(self, max_chat_id: str) -> bool:
        """Явный deny-by-default whitelist MAX-чатов."""
        return any(c.max_chat_id == str(max_chat_id) for c in self.chats)
...
def _resolve_user_id_set(value) -> frozenset[int]:
    """Прочитать только numeric Telegram user_id; пустое значение = deny."""
    if value is None:
        return frozenset()
    try:
        rendered = _resolve_env(str(value)).strip()
    except ValueError:
        # Whitelist is intentionally fail-closed when the optional env is absent.
        return frozenset()
    if not rendered:
        return frozenset()
    result: set[int] = set()
    for item in rendered.split(","):
        ...
        if not re.fullmatch(r"-?\d+", item):
            raise ValueError("TG_ALLOWED_USER_IDS must contain numeric IDs separated by commas")
        result.add(int(item))
...
def load_config(config_path: str = "config.yaml") -> AppConfig:
    ...
    tg = TelegramConfig(
        ...
        allowed_user_ids=_resolve_user_id_set(
            os.environ.get("TG_ALLOWED_USER_IDS", tg_raw.get("allowed_user_ids", ""))
        ),
        bot_username=_resolve_optional_env(
            os.environ.get("TG_BOT_USERNAME", tg_raw.get("bot_username"))
        ),
    ...
    br = BridgeConfig(
        forward_all=_resolve_bool(br_raw.get("forward_all"), False),
        outbound_enabled=_resolve_bool(
            os.environ.get("OUTBOUND_ENABLED", br_raw.get("outbound_enabled")),
            False,
        ),
