...
## Грабли при первом развёртывании

Собрано по живому запуску 2026-09-14. Каждый пункт стоил от получаса до
целого дня; проверить их дешевле, чем потом искать причину в журнале.

### 1. Сервер должен быть в России

MAX **не присылает SMS** на запросы с зарубежных адресов: авторизация
доходит до `StartAuthResponse`, а код не приходит. С российского адреса тот
же код, та же версия PyMax и тот же профиль устройства проходят с первого
запроса. Повторные попытки быстро дают `limit.violate` — ограничение на
стороне MAX, снимается ожиданием.

Проверять до первого `AUTH_REQUEST`: если предыдущие попытки шли через
прокси, MAX видит адрес прокси, а не адрес VPS.

### 2. ID форум-группы — с минусом

`TG_FORUM_GROUP_ID` берётся из ссылки на сообщение вида
`https://t.me/c/1234567890/5/12`: число после `/c/` плюс префикс `-100`,
итого `-1001234567890`. Без ведущего минуса Telegram отвечает
`Bad Request: chat not found` на создание темы, а в журнале моста —
`topic_create_failed` / `no_topic`.

Проверка до запуска:
```bash
curl -s "https://api.telegram.org/bot<TOKEN>/getChat?chat_id=<FORUM_ID>"
```

### 3. Бот — админ с правом «Управление темами»

Недостаточно быть администратором: нужен явный флаг `can_manage_topics`,
иначе тему под MAX-чат создать нельзя. Право выдаётся в Telegram и
подхватывается без перезапуска моста.

```bash
curl -s "https://api.telegram.org/bot<TOKEN>/getChatMember?chat_id=<FORUM_ID>&user_id=<BOT_ID>"
```

### 4. Реакции: Telegram запоминает набор подписок

`message_reaction` приходит только при явном `allowed_updates` в
`start_polling`, **и Telegram запоминает набор на своей стороне** до
следующего `getUpdates` с новым списком. Если реакции не приезжают —
сначала смотреть, что реально подписано:

```bash
curl -s "https://api.telegram.org/bot<TOKEN>/getWebhookInfo"
```

### 5. Права файлов конфигурации

Контейнер работает от UID 10001; `config.yaml` и `config.local*.yaml`
монтируются внутрь и читаются этим пользователем. Режим `600 root:root`
даёт `[Errno 13] Permission denied` и падение на старте. Верно:
`chown 10001:10001` + `chmod 400` для локального конфига, `444` для
`config.yaml`. Файлы `.env*` читает docker compose на хосте — им `600`.

### 6. Deny-by-default незаметен

Сообщение из Telegram уходит в MAX только если совпало всё: своя
форум-группа, отправитель в `TG_ALLOWED_USER_IDS`, упоминание бота в
начале, `OUTBOUND_ENABLED=true`. Любое несовпадение — тишина без ошибки,
так задумано. Новый участник = его Telegram user id (узнаётся через
[@userinfobot](https://t.me/userinfobot)) в список + `up -d`.

### 7. Чего MAX не присылает

- **Реакции MAX → Telegram** не приходят вовсе: подписка есть и в PyMax
  (`Client.on_reaction_update`), и в мосте, но событий нет. Обратное
  направление (Telegram → MAX) работает.
- **Удаление** в личных чатах приходит (статус `REMOVED`), в группах
  наблюдалось не всегда.

### 8. Что закрывается штатным Telegram

Отложенная отправка («зажать отправку → Отправить позже») работает без
доработок: Telegram доставляет сообщение в топик в назначенное время,
бот видит его как обычное входящее и пересылает в MAX. Упоминание бота в
отложенном сообщении сохраняется.

### 9. Перезапуск моста

Сообщения, отправленные в **группы** MAX пока мост остановлен, не
догоняются: `dm_history_sweep` работает только для личных чатов.
Перезапуски планировать в тихое время.
